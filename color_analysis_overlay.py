...
# 定義區域顏色 RGB 值（根據上傳的 TongueOverlay.png）
COLOR_TO_REGION = {
    (255, 0, 0): "腎",        # Blue in BGR
    (0, 255, 0): "脾胃",      # Green
    (0, 255, 255): "心肺",    # Lime (BGR)
    (128, 128, 128): "肝膽"   # Gray
}
...
def analyze_tongue_regions_with_overlay(photo_path, overlay_path="static/TongueOverlay.png"):
    ...
    if tongue_img is None or overlay_img is None:
        raise FileNotFoundError("無法讀取圖像或 overlay。")
    ...
    for color_bgr, region_name in COLOR_TO_REGION.items():
        ...
        advice = REGION_ADVICE_RULE.get(region_name, {}).get(diagnosis, "保持良好作息")
